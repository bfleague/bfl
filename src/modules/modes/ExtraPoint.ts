...
export class ExtraPoint extends Mode {
      ...
      if (
        ...
        if (
          this.game.teamWithBall === Team.Red
            ? player.getX() < MapMeasures.RedZoneBlue[1].x
            : player.getX() > MapMeasures.RedZoneRed[1].x
        ) {
          room.send({
            message: `❌ Tentativa de deixar a red zone por ${player.name} com a bola durante conversão • Perde a conversão`,
  ...
  public set({
    ...
    this.game.ballPosition = {
      team: this.game.invertTeam(forTeam),
      yards: yards ?? this.extraPointYards,
    };
    ...
    if (!silent) {
      ...
      for (let i = 0; i < kickingTeam.length; i++) {
        ...
        player.setX(
          ballPosInMap.x +
            (forTeam === Team.Red
              ? -this.playerBackDistanceExtraPoint
              : this.playerBackDistanceExtraPoint),
        );
      }
  ...
  private detectFailedExtraPoint(
    room: Room,
    player: Player,
  ...
  private handleSuccessfulExtraPoint(room: Room) {
    ...
    Utils.sendSoundTeamMessage(room, {
      message: `🙌 EXTRA POINT DO ${this.game.getTeamName(this.game.teamWithBall).toUpperCase()}!!! • +${this.epPoints} pontos para o ${this.game.getTeamName(this.game.teamWithBall)} • ${this.game.getScoreMessage()}`,
      color: Global.Color.LimeGreen,
      style: "bold",
    });
  }
  ...
  private handleMissedExtraPointBallWrongDirection(room: Room) {
    room.send({
      message: `🤖 Detectado Extra Point falho • Kickoff`,
  ...
  private handleMissedExtraPointBallStopped(room: Room) {
    ...
    room.send({
      message: `🥅 Errou Extra Point • Bola parou antes de chegar no gol`,
      color: Global.Color.Yellow,
      style: "bold",
    });
