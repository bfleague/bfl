  ...
  constructor(room: Room) {
    ...
    room.on("playerChat", (player, message) => {
      ...
      if (this.isUsingIllegalChars(message)) {
        player.reply({
          message: `⚠️ Mensagem bloqueada! Excesso de caracteres especiais`,
          sound: 2,
      ...
      if (
        !player.roles.includes(Global.loggedRole) &&
        !player.roles.includes(Global.bypassRegisterRole)
      ) {
        prefix = "✖️";
      ...
      const mentionedPlayers = room
        ...
        .map((p) => p.id);
      const msg = `${prefix} ${player.name}: ${message}`;
