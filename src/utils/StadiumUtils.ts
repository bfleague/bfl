...
export default class StadiumUtils {
  static addYardsToFieldPosition(
  ...
  static getCoordinateFromYards({
    team,
    yards,
  }: Global.FieldPosition): Position;
  static getCoordinateFromYards(team: Team, yards: number): Position;
  static getCoordinateFromYards(
    paramOne: Team | Global.FieldPosition,
    paramTwo?: number,
  ) {
    const { team, yards } =
      typeof paramOne === "object"
        ? paramOne
        : { team: paramOne, yards: paramTwo };

    if (team === Team.Red) {
      return {
        x: MapMeasures.RedEndZoneLineCenter.x + yards * MapMeasures.Yard,
        y: 0,
      };
    }

    return {
      x: MapMeasures.BlueEndZoneLineCenter.x - yards * MapMeasures.Yard,
      y: 0,
    };
  }
  ...
  static getDifferenceBetweenFieldPositions(
    pos1: Global.FieldPosition,
    pos2: Global.FieldPosition,
    teamAdvancing?: Team,
  ) {
    let yards1, yards2;
  ...
  static isInRedZone(pos: Global.FieldPosition, team: Team) {
    ...
    if (team === Team.Red) {
      ...
      return (
        coords.x >= redZone[0].x &&
        coords.x <= redZone[1].x &&
        coords.y >= redZone[0].y &&
        coords.y <= redZone[1].y
      );
    } else {
  ...
  static getYardsFromXCoord(x: number): Global.FieldPosition {
    const startX = MapMeasures.RedEndZoneLineCenter.x;
    const endX = MapMeasures.BlueEndZoneLineCenter.x;

    if (x < startX) return { team: Team.Red, yards: 1 };
    if (x > endX) return { team: Team.Blue, yards: 1 };

    const yardsFromCenter = Math.round(x / MapMeasures.Yard);
    const yardsComplement = 50 - Math.abs(yardsFromCenter);

    return {
      team: yardsFromCenter < 0 ? Team.Red : Team.Blue,
  ...
  static ballWithinGoalLine(ball: Disc, ofTeam: Team) {
    ...
    return (
      Math.abs(ballPos.x) > Math.abs(goalLine[0].x) &&
      ballPos.y > goalLine[0].y &&
      ballPos.y < goalLine[1].y
    );
  }
}
