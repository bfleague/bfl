...
export default class Invasion extends DownPlay {
  ...
  public handle(room: Room) {
    ...
    const { defendersCrowding, attackersCrowding } =
      this.getPlayersInvadingMid(room);
    ...
    this.crowdingPlayers = this.crowdingPlayers
      .map((p) => {
    ...
    crowdingDefendersIds.forEach((id) => {
      if (!oldCrowdersIds.includes(id)) {
        ...
        const playerHistoryIndex = this.crowdingPlayersHistory.findIndex(
          (p) => p[0] === id,
        );
    ...
    if (
      this.crowdingPlayers.every(
        (p) => p[1] < this.invasionTimeSeconds * Global.TICKS_PER_SECOND,
      )
    )
      return false;
    ...
    if (
      ...
      if (this.game.redZonePenalties >= this.game.down.maxPenaltiesInRedZone) {
        this.handleInvasionLines(room, () => {
          this.game.down.setRedZoneTouchdown(
            room,
            this.game.teamWithBall,
            this.game.invasionPlayers,
            `Invasão de ${Utils.getPlayersNames(this.game.invasionPlayers)}`,
          );
        });
        ...
        room.send({
          message: `⛔ Invasão de ${this.getInvasionPlayersString(room)} na Red Zone • ${this.game.redZonePenalties}/${this.game.down.maxPenaltiesInRedZone} para Touchdown Automático • ${penalty} jardas de penalidade`,
      ...
      room.send({
        message: `⛔ Invasão de ${this.getInvasionPlayersString(room)} • ${penalty} jardas de penalidade`,
        color: Global.Color.Orange,
        style: "bold",
      });
    }

    const sum = this.crowdingPlayersHistory.reduce(
    ...
    for (const p of defendersCrowding) {
      p.reply({
        message: `😡 Você não pode ficar no meio por mais que 3 segundos, a não ser que tenha jogadores do outro time também!`,
      ...
      const acc = parseFloat(
        (
          this.crowdingPlayersHistory.find((pl) => pl[0] === p.id)[1] / sum
        ).toFixed(2),
      );
    ...
    this.handleInvasionLines(room, () => {
      this.game.down.set({ room, decrement: penalty, countDown: false });
    });
  ...
  private getInvasionPlayersString(room: Room) {
    ...
    return Utils.getPlayersNames(
      this.crowdingPlayersHistory
        .sort((a, b) => b[1] - a[1])
        .map((p) => {
          const player = room.getPlayer(p[0]);
          ...
          return {
            name: `${player.name} (${((p[1] / sum) * 100).toFixed(1)}%)`,
          };
        })
        .filter((p) => p != null),
    );
  ...
  private getPlayersInvadingMid(room: Room) {
    ...
    const defendersCrowding = crowdingPlayers.filter(
      (p) => p.getTeam() !== this.game.teamWithBall,
    );
    const attackersCrowding = crowdingPlayers.filter(
      (p) => p.getTeam() === this.game.teamWithBall,
    );
  ...
  private arrangeLines(
    ...
    for (let i = 0; i < halfRectangle.length; i++) {
      const discIndex =
        measures.invasionLinesIndexes[invasionLineIndex][i % 2 === 0 ? 0 : 1];
  ...
  private setInvasionLines(room: Room) {
    ...
    room
      ...
      .setPosition({ x: behindBallPosX, y: hashHeightY1 });
    room
      .getDisc(this.invasionDiscsIndexes[1])
      .setPosition({ x: maxHorizontalX, y: hashHeightY1 });
    room
      .getDisc(this.invasionDiscsIndexes[2])
      .setPosition({ x: behindBallPosX, y: hashHeightY2 });
    room
      .getDisc(this.invasionDiscsIndexes[3])
      .setPosition({ x: maxHorizontalX, y: hashHeightY2 });
    ...
    if (this.isInnerInvasionValid()) {
      const innerTopLineY =
        (this.innerInvasionHeightYards / 2) * MapMeasures.Yard * -1;
      ...
      room
        ...
        .setPosition({ x: innerFrontX, y: innerTopLineY });
      room
        .getDisc(this.innerInvasionDiscsIndexes[1])
        .setPosition({ x: innerFrontX, y: innerBottomLineY });
