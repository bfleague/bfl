...
class CustomTeams extends Module {
  ...
  constructor(room: Room) {
    ...
    this.teamList = Object.entries(teams).map(([name, uniform]) => {
      ...
      return {
        ...
        colors: [uniformColor[2], uniformColor[3], uniformColor[4]].map((c) =>
          toColor(c),
        ),
      };
    });
    ...
    room.on("gameStart", () => {
      ...
      room.send({
        message: `​🏈​ ${this.teams.red.name} x ${this.teams.blue.name}`,
  ...
  private setNextGameTeams() {
    const maintainTeam = this.maintainTeam
      ? this.maintainTeam === Team.Red
        ? this.teams.red
        : this.teams.blue
      : null;
    ...
    this.teams = {
      red:
        this.maintainTeam === Team.Red ? maintainTeam : randomTeamSelection[0],
      blue:
        this.maintainTeam === Team.Blue ? maintainTeam : randomTeamSelection[1],
    };
  ...
  public swapTeams(room: Room) {
    ...
    if (this.maintainTeam)
      this.maintainTeam = this.maintainTeam === Team.Red ? Team.Blue : Team.Red;
  ...
  setteamCommand($: CommandInfo, room: Room) {
    ...
    let customTeam = this.teamList.find(
      (team) =>
        team.name
          ...
          .toLowerCase()
          .replace(/[^\S ]+/, "") ===
        customTeamStr?.toLowerCase()?.replace(/[^\S ]+/, ""),
    );
    ...
    if (teamStr === "red") {
      ...
      $.caller.reply({
        message: translate("INVALID_TEAM", room.prefix),
        sound: 2,
        color: Global.Color.Tomato,
        style: "bold",
      });
    ...
    if (!customTeam) {
      $.caller.reply({
        message: translate("TEAM_NOT_FOUND", room.prefix),
        sound: 2,
        color: Global.Color.Tomato,
        style: "bold",
      });
  ...
  teamlistCommand($: CommandInfo, room: Room) {
    $.caller.reply({
      message: translate(
        ...
        this.teamList.map((p) => p.name).join(", "),
      ),
      color: Global.Color.Tomato,
      style: "bold",
    });
