...
export class AFK extends Module {
    ...
    room.on("playerTeamChanged", (changedPlayer: Player) => {
      if (changedPlayer.getTeam() !== Team.Spectators) {
        if (changedPlayer.settings.afk) {
          ...
          room.send({
            message: `⚠️ ${changedPlayer.name} está AFK e não pode ser movido!`,
  ...
  afkCommand($: CommandInfo, room: Room) {
    if (
      ...
      $.caller.getTeam() !== Team.Spectators &&
      !room.isGamePaused() &&
      room.getPlayers().teams().length > 2
    ) {
      $.caller.reply({
        message: "⚠️ Você não pode ficar AFK com o jogo despausado!",
        color: Global.Color.Tomato,
        style: "bold",
      });
    ...
    if ($.caller.settings.afk) {
      ...
      room.send({
        message: `🥱 ${$.caller.name} não está mais AFK!`,
        color: Global.Color.SeaGreen,
        style: "bold",
      });
      ...
      if ($.caller.getTeam() !== Team.Spectators && room.isGameInProgress()) {
        $.caller.reply({
          message: "⚠️ Você não pode ficar AFK durante a partida!",
          color: Global.Color.Tomato,
          style: "bold",
        });
      ...
      if ($.caller.getTeam() !== Team.Spectators)
        $.caller.setTeam(Team.Spectators);
    }
  ...
  afksCommand($: CommandInfo, room: Room) {
    ...
    if (afks.length > 0) {
      $.caller.reply({
        message: `😴 Jogadores AFK (${afks.length}): ${Utils.getPlayersNames(afks)}`,
        color: Global.Color.MediumSeaGreen,
        style: "bold",
      });
    } else {
      $.caller.reply({
        message: `⚠️ Não há nenhum jogador AFK!`,
        color: Global.Color.Tomato,
        style: "bold",
      });
    }
