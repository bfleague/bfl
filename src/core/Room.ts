...
declare interface Room {
  on(
    event: `${EventNames.onPlayerJoin}`,
    listener: (player: Player) => void,
  ): this;
  on(
    event: `${EventNames.onPlayerLeave}`,
    listener: (player: Player) => void,
  ): this;
  on(
    event: `${EventNames.onTeamVictory}`,
    listener: (scores: ScoresObject) => void,
  ): this;
  on(
    event: `${EventNames.onPlayerChat}`,
    listener: (player: Player, message: string, isCommand: boolean) => boolean,
  ): this;
  on(
    event: `${EventNames.onPlayerBallKick}`,
    listener: (player: Player) => void,
  ): this;
  on(event: `${EventNames.onTeamGoal}`, listener: (team: TeamID) => void): this;
  on(
    event: `${EventNames.onGameStart}`,
    listener: (byPlayer: Player) => void,
  ): this;
  on(
    event: `${EventNames.onGameStop}`,
    listener: (byPlayer: Player) => void,
  ): this;
  on(
    event: `${EventNames.onPlayerAdminChange}`,
    listener: (changedPlayer: Player, byPlayer: Player) => void,
  ): this;
  on(
    event: `${EventNames.onPlayerTeamChange}`,
    listener: (changedPlayer: Player, byPlayer: Player) => void,
  ): this;
  on(
    event: `${EventNames.onPlayerKicked}`,
    listener: (
      kickedPlayer: Player,
      reason: string,
      ban: boolean,
  ...
  on(
    event: `${EventNames.onGamePause}`,
    listener: (byPlayer: Player) => void,
  ): this;
  on(
    event: `${EventNames.onGameUnpause}`,
    listener: (byPlayer: Player) => void,
  ): this;
  on(event: `${EventNames.onPositionsReset}`, listener: () => void): this;
  on(
    event: `${EventNames.onPlayerActivity}`,
    listener: (player: Player) => void,
  ): this;
  on(
    event: `${EventNames.onStadiumChange}`,
    listener: (newStadiumName: string, byPlayer: Player) => void,
  ): this;
  on(event: `${EventNames.onRoomLink}`, listener: (url: string) => void): this;
  on(
    event: `${EventNames.onKickRateLimitSet}`,
    listener: (
      min: number,
      rate: number,
      burst: number,
  ...
  on(
    event: `${EventNames.onBeforeEstablishConnection}`,
    listener: (ip: string) => boolean,
  ): this;
  on(
    event: "playerNeedsConfirmation",
    listener: (player: Player) => void,
  ): this;
  on(
    event: "newPlayerTouchBall",
    listener: (player: Player, before?: Player) => void,
  ): this;
  on(
    event: "gameStartTicking",
    listener: (player: Player, before?: Player) => void,
  ): this;
  on(
    event: "roomAnnouncement",
    listener: (announcement: MessageObject) => void,
  ): this;
...
class Room extends EventEmitter {
  ...
  private _specialEvents = {
    [EventNames.onGameStart]: (byPlayer: PlayerObject) => {
      ...
      for (
        let i = 0;
        i < this._room.getDiscCount() - this.getPlayers().teams().length;
        i++
      ) {
    ...
    [EventNames.onPlayerChat]: (playerObj: PlayerObject, message: string) => {
      ...
      if (message[0] === this.prefix && command && player.canUseCommands) {
        ...
        if (
          roles == null ||
          roles.length == 0 ||
          roles.filter((r) => (player.roles ?? []).includes(r)).length > 0
        ) {
    ...
    [EventNames.onGameTick]: () => {
      ...
      if (players.length > 1) {
        ...
        this._ballCollisions[0]?.player &&
        players[0].id !== this._ballCollisions[0].player.id
      ) {
        this._ballCollisions[1] = {
          player: this._ballCollisions[0].player,
          time: this._ballCollisions[0].time,
        };
  ...
  private load() {
    for (const [key, val] of Object.entries(EventNames)) {
      this._room[key] = (...args: any) => {
        ...
        if (
          val === EventNames.onPlayerJoin &&
          this._playerConfirmLevels.length > 0
        )
          return;
        ...
        args = args.map((a: any) => {
          if (PlayerList.isPlayerObject(a)) {
            if (val === EventNames.onPlayerLeave) {
              ...
              const p =
                players.get(a.id) ?? this._deletedPlayers.get(a.id) ?? a;
  ...
  public removeCommand(commandName: string) {
    this._commands = this._commands.filter(
      (command) => Reflect.getMetadata("command:name", command) === commandName,
    );
  }
  ...
  public removeConfirmLevel(name: string) {
    this._playerConfirmLevels = this._playerConfirmLevels.filter(
      (c) => c !== name,
    );
  }
  ...
  public setStadium(stadium: {} | DefaultStadiums): void {
    ...
    if (typeof stadium === "string")
      this._room.setDefaultStadium(stadium as DefaultStadiums);
  }
  ...
  public setTeamColors(team: TeamID | "all", teamColors: TeamColors): void {
    if (team === "all") {
      this._room.setTeamColors(
        Team.Red,
        teamColors.angle,
        teamColors.textColor,
        teamColors.colors,
      );
      this._room.setTeamColors(
        Team.Blue,
  ...
  public send(options: MessageObject): void {
    ...
    this.emit("roomAnnouncement", options);
  }
