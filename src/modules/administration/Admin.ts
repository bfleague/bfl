...
export class Admin extends Module {
  ...
  constructor(room: Room) {
    ...
    room.on("playerKicked", (kickedPlayer, reason, ban, byPlayer) => {
      ...
      if (
        ...
        byPlayer.kick(
          "Você não pode banir registrados sem ser um admin oficial!",
        );
      ...
      const player = room
        ...
        .filter(
          (p) =>
            ...
            !p.settings.afk &&
            (this.restrictNonRegisteredPlayers
              ? p.roles.includes(Global.loggedRole)
              : true),
        )[0];
  ...
  desbanirCommand($: CommandInfo, room: Room) {
    ...
    const bannedPlayers = this.bans.filter(
      (p) => p.id === parseInt(idOrName) || p.name === idOrName,
    );
    ...
    if (bannedPlayers.length) {
      ...
      room.send({
        message: `🕊️ ${message} por ${$.caller.name}`,
        color: Global.Color.HotPink,
        style: "bold",
      });
    } else {
      $.caller.reply({
        message: `⚠️ Não foi encontrado nenhum jogador banido com esse nome ou ID!`,
        color: Global.Color.Tomato,
        style: "bold",
      });
    }
